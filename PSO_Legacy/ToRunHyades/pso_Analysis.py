...
print("THIS IS 100% THE RIGHT ANALYSIS FILE!!!")
...
def Gain(file, CBET_MULTIPLIER=0.8, AH_energy_J=0, AH_efficiency=0.094):
    ...
    if np.any(Neutrons > (max(Neutrons)*0.9999)):
        ...
        NeutronGain = NeutronEnergy / RequiredLaserEnergy
...
def ConvergenceRatio(file):
    ...
    # CR at all timesteps
    OlsonCRTimeSeries = InitialIceBoundaryRadius / OuterShellRadii
    LindlCRTimeSeries = X[0][-1] / OuterShellRadii
...
def LaserProfile(file, CBET_MULTIPLIER=0.8):
    ...
    return SimulationLaserPower, ExperimentalLaserPower, ParametricLimit
